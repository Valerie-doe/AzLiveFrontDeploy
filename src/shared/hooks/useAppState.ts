...
export function useAppState() {
  ...
  const [isAuthenticated, setIsAuthenticated] = useState<boolean>(() => {
    return localStorage.getItem('azlive_authenticated') === 'true';
  });
    ...
    const interval = setInterval(() => {
      ...
      if (variant) {
        const commentTemplates = [
          `${product.jpCode} ${sizeText} ${colorText} s'il vous plaît!`,
          `${product.jpCode} en ${colorText} taille ${sizeText} je prends`,
          `Manao commande de ${product.jpCode} (${sizeText} - ${colorText})`,
          `${product.jpCode} kely ${colorText} size ${sizeText} band`,
          `Afaka mahazo ${product.jpCode} kosa ve, taille ${sizeText} ${colorText}?`,
        ];
        ...
        customComment = keywordSample
          .replace('JP1', product.jpCode)
          .replace('JP2', product.jpCode)
          .replace('JP3', product.jpCode)
          .replace('JP4', product.jpCode)
          .replace('JP5', product.jpCode);
      }
      ...
      const simulatedClaim: Order = {
        ...
        chatMessage: `Bonjour 👋 Vous avez écrit "${customComment}". Votre article ${
          ...
          product.jpCode
        }) est réservé pour ${product.price.toLocaleString()} Ar. Merci de nous envoyer votre NOM, TÉLÉPHONE, ADRESSE et d'effectuer le transfert par Mobile Money (Mvola, Orange Money ou Airtel) pour valider !`,
        remindersCount: 0,
  ...
  const handleAddManualOrder = (
    ...
    const product = products.find((p) => p.jpCode === jpCode);
    if (!product) return;
    ...
    const manualClaim: Order = {
      ...
      chatMessage: `Bonjour 👋 Votre commande pour ${
        product.name
      } (${jpCode}) est réservée pour ${product.price.toLocaleString()} Ar. Merci d'envoyer votre reçu de paiement Mobile Money pour valider !`,
      remindersCount: 0,
      deliveryStatus: 'au bureau',
  ...
  const handleLoginSuccess = (pages: string[]) => {
    localStorage.setItem('azlive_authenticated', 'true');
  ...
  const handleConfirmLogout = () => {
    localStorage.removeItem('azlive_authenticated');
