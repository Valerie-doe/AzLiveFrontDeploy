...
export default function ManualOrderForm({ products, onSubmit }: ManualOrderFormProps) {
  const [selectedJpCode, setSelectedJpCode] = useState<string>(products[0]?.jpCode || 'JP1');
  const [manualCustomer, setManualCustomer] = useState<string>('');

  const currentProduct = products.find(p => p.jpCode === selectedJpCode);
        ...
        <div>
          ...
          {currentProduct && (
            <div className="mt-2 text-[11px] bg-slate-50 p-2.5 rounded-lg text-slate-600 flex justify-between">
              ...
              <span className="font-mono text-indigo-600 font-extrabold">{currentProduct.price.toLocaleString()} Ar</span>
            </div>
          )}
        </div>
