...
import { Image as ImageIcon, Printer, Edit3, Trash2 } from 'lucide-react';
    ...
    >
      ...
      <div className="flex justify-between items-start">
        <div>
          <span className="font-mono text-[9px] bg-indigo-600 text-white px-2 py-0.5 rounded-md font-bold uppercase select-none">
            Code : {selectedProduct.jpCode}
          </span>
          <h3 className="font-black text-slate-900 text-base mt-1.5 leading-snug">{selectedProduct.name}</h3>
          <p className="text-lg font-black text-indigo-600 mt-1 font-sans">{selectedProduct.price.toLocaleString()} Ar</p>
        </div>
        <div className="h-14 w-14 bg-slate-100 border rounded-xl overflow-hidden flex-shrink-0 flex items-center justify-center">
      ...
      <div className="grid grid-cols-3 gap-2 py-1 text-center">
        <div className="bg-slate-50 p-2 rounded-xl border border-slate-100 text-xs">
          ...
          <span className="font-extrabold text-slate-800">{selectedProduct.size}</span>
        </div>
        <div className="bg-slate-50 p-2 rounded-xl border border-slate-100 text-xs">
          <span className="text-[8px] font-mono uppercase text-slate-400 font-bold block">Couleur</span>
          <span className="font-extrabold text-slate-800 truncate block max-w-full">{selectedProduct.color || 'Standard'}</span>
        </div>
        <div className="bg-slate-50 p-2 rounded-xl border border-slate-100 text-xs">
      ...
      {/* Sizing Details Table if Variants Exist */}
      {selectedProduct.variants && selectedProduct.variants.length > 0 && (
        <div className="bg-slate-50/50 p-3 rounded-2xl border border-slate-100 space-y-2">
          <span className="text-[9px] font-mono uppercase text-slate-400 font-bold block">
            Détail de l'Inventaire par Variante ({selectedProduct.variants.length})
          </span>
            ...
            <table className="w-full text-left text-[10px] text-slate-700">
              ...
              <tbody className="divide-y divide-slate-50">
                {selectedProduct.variants.map((v, idx) => (
                  <tr key={v.id || idx} className="hover:bg-slate-50/20">
      ...
      <div className="space-y-3">
        ...
        <div className="bg-white border-2 border-slate-950 p-4 rounded-xl text-center flex flex-col items-center justify-center relative overflow-hidden select-all shadow-sm">
          <div className="absolute top-1 left-2 text-[7px] font-mono text-slate-400 uppercase">Sticker Adhésif</div>
          <div className="absolute top-1 right-2 text-[7px] font-mono text-slate-400 uppercase">AZLive Tech</div>

          <div className="absolute left-0 right-0 top-3 border-t border-dotted border-slate-300"></div>

          <div className="my-2.5 text-slate-950">
            <div className="text-xl font-black font-mono tracking-tight uppercase">
              {selectedProduct.jpCode} {selectedProduct.name}
            </div>
          ...
          <div className="w-full mt-2.5 pt-2 border-t border-dashed border-slate-200 text-[9px] text-slate-500 font-mono">
            <span>Options : {selectedProduct.size} • {selectedProduct.color || 'Unique'}</span>
          </div>
        </div>
      </div>
      ...
      <div className="pt-2 flex flex-col gap-2">
        <button
          onClick={() => {
            alert(`Action: Impression thermique d'étiquette lancée !\n\nTitre: ${selectedProduct.jpCode} ${selectedProduct.name} – ${selectedProduct.price.toLocaleString()} Ar\nFormat: Adhésif 50x30mm`);
          }}
          className="w-full py-2 bg-slate-900 hover:bg-slate-800 text-white font-mono text-xs uppercase tracking-wider font-extrabold rounded-xl flex items-center justify-center gap-1.5 transition-all cursor-pointer text-center border-none"
        >
