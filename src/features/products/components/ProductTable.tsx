...
import { getProductDisplay } from '../utils/productUtils';
...
export default function ProductTable({
  ...
  const filteredProducts = products.filter((p) => {
    ...
    return (
      ...
      display.jpCode.toLowerCase().includes(q) ||
      display.size.toLowerCase().includes(q) ||
      display.color.toLowerCase().includes(q) ||
      p.variants.some((v) => v.jpCode.toLowerCase().includes(q))
    );
  });
                  ...
                  >
                    ...
                    <td className="py-3 px-5 text-center">
                      <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold ${
                        ...
                        {display.stock} restant{display.stock > 1 ? 's' : ''}
                      </span>
                    </td>
