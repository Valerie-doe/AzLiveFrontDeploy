...
export default function ProductForm({
  ...
  const [currency, setCurrency] = useState('Ar'); // Madagascar Ariary Ar
  const [stock, setStock] = useState<number>(10);
  const [imageUrl, setImageUrl] = useState('');

  // Sizing and Color Variation states
  const [useVariants, setUseVariants] = useState(false);
  const [formVariants, setFormVariants] = useState<{ id: string; size: string; color: string; stock: number }[]>([]);
  ...
  useEffect(() => {
    if (productToEdit) {
      ...
      setJpCode(productToEdit.jpCode);
      ...
      if (productToEdit.variants && productToEdit.variants.length > 0) {
        setUseVariants(true);
        setFormVariants(productToEdit.variants);
      } else {
      ...
      setStock(10);
      setImageUrl('');
      setUseVariants(true); // default to variations
      setFormVariants([]);
    }
    setNewVarSize('M');
    setNewVarColor('');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim() || !jpCode.trim() || price <= 0) {
      alert('Veuillez remplir le nom du produit, un code JP valide (e.g. JP1) et un prix positif.');
      return;
    }

    let finalStock = Number(stock);
    let finalSizes = size.trim() || 'Freesize';
    let finalColors = color.trim() || 'Unique';
    ...
    if (useVariants) {
      ...
      finalStock = formVariants.reduce((sum, v) => sum + v.stock, 0);

      const distinctSizes = Array.from(new Set(formVariants.map(v => v.size.trim()).filter(Boolean)));
      finalSizes = distinctSizes.join(', ') || 'Freesize';

      const distinctColors = Array.from(new Set(formVariants.map(v => v.color.trim()).filter(Boolean)));
      finalColors = distinctColors.join(', ') || 'Unique';
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="space-y-3.5 text-xs">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label htmlFor="form-price" className="text-slate-400 block mb-1 font-mono uppercase text-[9px] font-bold">Prix d'Enchère*</label>
              <input
                id="form-price"
                type="number"
                required
                ...
                className="w-full px-3 py-2 border rounded-xl focus:ring-1 focus:ring-indigo-500 focus:outline-none font-black text-slate-800 text-xs bg-white border-slate-200"
              />
            </div>
            <div>
              <label htmlFor="form-currency" className="text-slate-400 block mb-1 font-mono uppercase text-[9px] font-bold">Devise</label>
              <select
                id="form-currency"
                value={currency}
                onChange={(e) => setCurrency(e.target.value)}
                className="w-full px-3 py-2 border rounded-xl bg-slate-50 focus:ring-1 focus:ring-indigo-500 focus:outline-none font-semibold text-slate-800 text-xs border-slate-200"
              >
          ...
          <div>
            <label htmlFor="form-imageUrl" className="text-slate-400 block mb-1 font-mono uppercase text-[9px] font-bold">URL Image</label>
            <input
              id="form-imageUrl"
              type="url"
              ...
              className="w-full px-3 py-2 border rounded-xl focus:ring-1 focus:ring-indigo-500 focus:outline-none text-slate-700 text-xs truncate border-slate-200 bg-white"
            />
          </div>
        </div>
