...
interface CollaboratorFormProps {
  onSubmit: (collab: Omit<Collaborator, 'id' | 'stats' | 'isActive'>) => void | Promise<void>;
  onClose: () => void;
  saving?: boolean;
}

export default function CollaboratorForm({ onSubmit, onClose, saving = false }: CollaboratorFormProps) {
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-200 flex items-center justify-center p-4 transition-all" id="recruitment-modal">
      ...
      <motion.div
        ...
        className="relative w-full max-w-lg bg-linear-to-tr from-slate-900 to-indigo-950 text-white rounded-3xl p-6 shadow-2xl border border-slate-800 space-y-5 overflow-y-auto max-h-[90vh] z-10"
      >
        {/* Close button X */}
        <button
        ...
        <form onSubmit={handleFormSubmit} className="space-y-4 text-xs">
          ...
          <div>
            ...
            <button
              type="submit"
              disabled={saving}
              className="flex-2 py-3 bg-linear-to-r from-emerald-500 to-teal-500 text-slate-950 font-black rounded-xl text-xs uppercase tracking-wider shadow-lg hover:scale-[1.01] transition-all cursor-pointer border-none flex items-center justify-center gap-1.5 disabled:opacity-60"
            >
              <Sparkles className="h-4 w-4" /> {saving ? 'Enregistrement...' : "Intégrer l'agent au secrétariat"}
            </button>
          </div>
        </form>
      </motion.div>
