...
export interface TestCommentState {
  pageId: string;
  customerName: string;
...
const INITIAL_CHANNELS: ConnectedChannel[] = [
  {
    id: 'ch-fb-1',
    name: 'AZLive Fashion',
    platform: 'Facebook',
...
const INITIAL_LOGS: SandboxLogEntry[] = [
  {
    id: 'log-1',
    time: '12:20:45',
    page: 'AZLive Fashion',
...
export function useIntegrationsState(
  ...
  const [channels, setChannels] = useState<ConnectedChannel[]>(INITIAL_CHANNELS);
  ...
  const [testComment, setTestComment] = useState<TestCommentState>({
    pageId: 'ch-fb-1',
  ...
  const [sandboxLog, setSandboxLog] = useState<SandboxLogEntry[]>(INITIAL_LOGS);
  const [webhookAlert, setWebhookAlert] = useState<string | null>(null);

  const handleOpenAuth = (platform: 'Facebook' | 'TikTok') => {
    playNotificationSound('click');
    setAuthModal({ isOpen: true, platform, step: 1, selectedPageChoice: [] });
  };

  const handleNextStep = () => {
    playNotificationSound('click');
    setAuthModal(prev => ({ ...prev, step: (prev.step + 1) as 2 | 3 }));
  };

  const handlePrevStep = () => {
    playNotificationSound('click');
    setAuthModal(prev => ({ ...prev, step: (prev.step - 1) as 1 | 2 }));
  };
  ...
  const handleCompleteIntegration = () => {
    ...
    const chosenPages = authModal.selectedPageChoice.length > 0
      ? authModal.selectedPageChoice
      : (platform === 'Facebook' ? ['Dressing de Riana'] : ['@mada_fripe_chic']);
    ...
    setAuthModal({ isOpen: false, platform: null, step: 1, selectedPageChoice: [] });
  ...
  const handleToggleWebhook = (id: string) => {
    playNotificationSound('click');
    setChannels(prev => prev.map(c => c.id === id ? { ...c, webhookActive: !c.webhookActive } : c));
  };
  ...
  const handleFireWebhookMock = (e: React.FormEvent) => {
    ...
    if (!matchedChannel.webhookActive) {
      setWebhookAlert(`⚠️ Le Webhook pour la page "${matchedChannel.name}" est désactivé. Veuillez l'activer pour recevoir les requêtes.`);
      setTimeout(() => setWebhookAlert(null), 4000);
      return;
    }
    ...
    setWebhookAlert(`🟢 Webhook reçu ! Commentaire capturé de ${testComment.customerName} sur code [${code}].`);
    setTimeout(() => setWebhookAlert(null), 5000);
    setTestComment(prev => ({ ...prev, commentText: '' }));
  };
