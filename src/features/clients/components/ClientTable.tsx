...
function formatLiveAssocie(liveTitles: string[]): string {
  if (liveTitles.length === 0) return '—';
  if (liveTitles.length === 1) return liveTitles[0];
  return `${liveTitles[0]} +${liveTitles.length - 1}`;
}
...
export default function ClientTable({
  ...
  // Colonnes : Nom FB, FB ID, Commandes, Articles, Montant, Dernière, [Live], Fiche.
  const columnCount = showLiveColumn ? 8 : 7;
        ...
        <table className="w-full text-left text-xs text-slate-650">
          <thead>
            <tr className="bg-slate-50/70 border-b border-slate-100 text-slate-400 font-mono text-[9px] uppercase tracking-wider">
              <th className="py-3 px-5">Nom Facebook</th>
              <th className="py-3 px-5">Facebook User ID</th>
              <th className="py-3 px-5 text-center">Commandes</th>
              <th className="py-3 px-5 text-center">Articles</th>
              <th className="py-3 px-5 text-right">Montant total</th>
              <th className="py-3 px-5">Dernière commande</th>
              {showLiveColumn && <th className="py-3 px-5">Live associé</th>}
          ...
          <tbody className="divide-y divide-slate-100 font-sans text-xs">
            {loading ? (
              <tr>
                <td colSpan={columnCount} className="py-12 text-center text-slate-400 font-mono">
              ...
              <tr>
                <td colSpan={columnCount} className="py-12 text-center text-slate-400 font-mono">
                  Aucun client détecté pour cette sélection.
                </td>
              </tr>
            ) : (
              displayedClients.map((client, idx) => (
                ...
                >
                  ...
                  <td className="py-3.5 px-5 font-mono text-[11px] text-slate-500 max-w-[140px] truncate" title={client.facebookId || ''}>
                    {client.facebookId || '—'}
                  </td>
                  ...
                  <td className="py-3.5 px-5 text-center font-bold text-slate-700">{client.itemsCount}</td>
                  ...
                  {showLiveColumn && (
                    <td className="py-3.5 px-5 max-w-[160px] truncate font-serif text-slate-500" title={client.liveTitles.join(', ')}>
                      {formatLiveAssocie(client.liveTitles)}
                    </td>
                  )}
